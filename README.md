...
* **Python 3.2+**: The main benchmark script is written with the programming language python: The benchmark script by default uses the version of Python on your path.
* **Python-yaml**: PyYAML is a YAML parser and emitter for Python. We've picked YAML as the configuration file format for specifying the structure for the project.
* **SQLite**: SQLite is a lightweight disk-based database that doesn’t require a separate server process. We use the python built-in SQLite database to save the benchmark results.
* **Valgrind** (Optional): Valgrind is a suite of tools for debugging and profiling. This package is only needed if you want to run the memory benchmarks.
