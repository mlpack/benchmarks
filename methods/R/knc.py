...
class KNC(object):
  ...
  '''
  KNC. If the method has been successfully completed return
  ...
  def RunMetrics(self, options):
    ...
    self.build_opts = {}
    if "k" in options:
      self.build_opts["k"] = int(options.pop("k"))
    else:
    ...
    cmd = shlex.split("libraries/bin/Rscript " + self.path + "knc.r" +
        " -t " + self.dataset[0] + " -T " +
        self.dataset[1] + " -k " + str(self.build_opts["k"]))
