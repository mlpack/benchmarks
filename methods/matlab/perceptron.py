'''
  ...
  Class to benchmark the matlab Pereptron method.
'''
...
class PERCEPTRON(object):
  ...
  def __del__(self):
    ...
    filelist = ["matlab_pc_predictions.csv"]
    for f in filelist:
      if os.path.isfile(f):
        os.remove(f)
  '''  
  '''
  Perceptron benchmark instance. If the method has been successfully
  ...
  def RunTiming(self, options):
    ...
    cmd = shlex.split(self.path + "matlab -nodisplay -nosplash -r \"try, " +
        "PERCEPTRON('"  + inputCmd + "'), catch, exit(1), end, exit(0)\"")
  ...
  def RunMetrics(self, options):
    if len(self.dataset) == 3:
      ...
      if not CheckFileAvailable("matlab_pc_predictions.csv"):
        self.RunTiming(options)
        
      testData = LoadDataset(self.dataset[1])
      truelabels = LoadDataset(self.dataset[2])

      probabilities = LoadDataset("matlab_pc_predictions.csv")
