...
class ALLKNN(object):
  ...
  def RunMemoryProfiling(self, options, fileName, massifOptions="--depth=2"):
    ...
    if len(self.dataset) == 2:
      cmd = shlex.split(self.debug + "allknn -r " + self.dataset[0] + " -q " + 
          self.dataset[1] + " -v -n neighbors.csv -d distances.csv " + options)
    else:
      cmd = shlex.split(self.debug + "allknn -r " + self.dataset + 
  ...
  def parseTimer(self, data):
    ...
    if not match:
      ...
      return timer(float(match.group("loading_data")),
          float(match.group("saving_data")), float(match.group("total_time")))
