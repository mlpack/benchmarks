'''
  ...
  Class to benchmark the mlpack mlp forward pass.
'''
...
'''
This class implements the MLP forward benchmark.
'''
class MLP_FORWARD(object):

  '''
  Create the mlp forward benchmark instance, show some informations and
  return the instance.
  ...
  def __init__(self, dataset, timeout=0, path=os.environ["MLPACK_BIN_SRC"],
    ...
    cmd = shlex.split(self.path + "mlp_forward -h")
    try:
  ...
  def RunMetrics(self, options):
    ...
    if timer != -1:
      metrics['Runtime'] = timer.forward
      metrics['Forward'] = timer.forward
  ...
  def parseTimer(self, data):
    ...
    if not match:
      ...
      timer = collections.namedtuple("timer", ["forward", "model_building",
          "total_time"])

      return timer(float(match.group("forward")),
