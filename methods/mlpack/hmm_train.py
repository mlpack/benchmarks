...
class HMMTRAIN(object):
  ...
  def RunMemoryProfiling(self, options, fileName, massifOptions="--depth=2"):
    Log.Info("Perform HMM Training Memory Profiling.", self.verbose)

    # If the dataset contains two files then the second file is the query file. 
    # In this case we add this to the command line.
    if len(self.dataset) == 2:
      cmd = shlex.split(self.debug + "hmm_train -i " + self.dataset[0] + "-l " + 
          self.dataset[1] + " -v " + options)
    else:
      cmd = shlex.split(self.debug + "hmm_train -i " + self.dataset + 
          " -v  " + options)   
