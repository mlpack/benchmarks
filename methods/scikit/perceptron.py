...
'''
class SCIKIT_PERCEPTRON(object):
  def __init__(self, method_param, run_param):
    self.info = "SCIKIT_PERCEPTRON ("  + str(method_param) +  ")"
    ...
    if "max_iterations" in method_param:
      self.build_opts["n_iter"] = int(method_param["max_iterations"])
  ...
  def metric(self):
    totalTimer = Timer()
    with totalTimer:
      model = Perceptron(**self.build_opts)
